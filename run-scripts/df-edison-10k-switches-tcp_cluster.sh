...
mpirun --hostfile $HOSTFILE --mca btl_tcp_if_include eno2np1 -np $RANKS $BIN --sync=3 --lp-io-dir=$LP_IO_PARENT_DIR/$LP_IO_DIR --lp-io-use-suffix=1 -- $CONFIG | tee "$OUT_DIR/$LP_IO_DIR.txt"
